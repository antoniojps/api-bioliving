...
$app->get('/eventos/{id}/interesse', function (Request $request, Response $response) {
    ...
    if (Token::validarScopes('admin', Token::getUtilizador())) {
        ...
        if (is_int($idEventos) && $idEventos > 0 && is_int($idUtilizador) && $idUtilizador) {
            ...
            $sql = "SELECT COUNT(`eventos_id_eventos`) AS interessado FROM `interesses` WHERE`eventos_id_eventos`= :idEventos AND `utilizadores_id_utilizadores`= :idUtilizadores";
...
//////////// Obter utilizadores que interessados num evento ////////////
# Parametros:
