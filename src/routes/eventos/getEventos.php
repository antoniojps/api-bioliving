    ...
    if (Token::validarScopes('admin')) {
        ...
        if ($page > 0 && $results > 0) {
            ...
            if ($order == $orderDefault) {
                $sql = "SELECT eventos.*,tipo_evento.nome_tipo_evento,icons.classe AS tipo_classe, COUNT(participantes.utilizadores_id_utilizadores) AS inscritos, COUNT(interesses.utilizadores_id_utilizadores) AS interessados ,COUNT(validaparticipantes.utilizadores_id_utilizadores) AS participantes ,localizacao.nome,localizacao.lat,localizacao.lng FROM eventos LEFT OUTER JOIN localizacao ON eventos.localizacao_localizacao = localizacao.localizacao LEFT OUTER JOIN participantes ON eventos.id_eventos = participantes.eventos_id_eventos LEFT OUTER JOIN interesses ON eventos.id_eventos = interesses.eventos_id_eventos LEFT OUTER JOIN validaparticipantes ON eventos.id_eventos = validaparticipantes.eventos_id_eventos LEFT OUTER JOIN tipo_evento ON eventos.tipo_evento_id_tipo_evento = tipo_evento.id_tipo_evento LEFT OUTER JOIN icons ON icons.id_icons = tipo_evento.icons_id WHERE (nome_evento LIKE :msg OR data_evento LIKE :msg OR descricao_short LIKE :msg OR tipo_evento.nome_tipo_evento LIKE :msg OR localizacao.nome LIKE :msg) AND id_eventos LIKE :id GROUP BY id_eventos ORDER BY $passar  LIMIT :limit , :results";
            } else {
...
$app->get('/eventos/{id}', function (Request $request, Response $response) {
    ...
    if (is_int($id) && $id > 0) {
        ...
        $sql = "SELECT eventos.*,tipo_evento.nome_tipo_evento as tipo,localizacao.*,COUNT(participantes.utilizadores_id_utilizadores) as inscritos,facebook_id,COUNT(interesses.utilizadores_id_utilizadores) as interessados,facebook_id,COUNT(validaparticipantes.utilizadores_id_utilizadores) as participantes,facebook_id FROM eventos LEFT OUTER JOIN localizacao ON eventos.localizacao_localizacao = localizacao.localizacao LEFT OUTER JOIN participantes ON eventos.id_eventos = participantes.eventos_id_eventos LEFT OUTER JOIN interesses ON eventos.id_eventos = interesses.eventos_id_eventos LEFT OUTER JOIN validaparticipantes ON eventos.id_eventos = validaparticipantes.eventos_id_eventos LEFT OUTER JOIN tipo_evento ON eventos.tipo_evento_id_tipo_evento = tipo_evento.id_tipo_evento  WHERE id_eventos = :id ";
...
$app->get('/eventos/{id}/extras', function (Request $request, Response $response) {
    ...
    $byDefault = 'titulo'; // order by predefinido
    $byArr = [
        'id' => 'id_extras',
        'titulo' => 'titulo'
    ];
    ...
    if ($page > 0 && $results > 0) {
        ...
        if ($dataMin || $dataMax) {
            ...
            if ($order == $orderDefault) {
                ...
                $sql = "SELECT id_eventos,criador,nome_evento,nome_tipo_evento,morada,data_evento,descricao_short,   inscritos, interessados, participantes,lat,lng,facebook_id,tipo_classe FROM( 
SELECT id_eventos, criador,nome_evento,nome_tipo_evento,morada,data_evento,descricao_short,  inscritos,  interessados,participantes,lat,lng,facebook_id, tipo_classe FROM(
 SELECT id_eventos,eventos.utilizadores_id_utilizadores AS criador,nome_evento,tipo_evento.nome_tipo_evento,localizacao.nome AS morada,data_evento,descricao_short, COUNT(DISTINCT participantes.utilizadores_id_utilizadores) AS inscritos,COUNT(DISTINCT validaparticipantes.utilizadores_id_utilizadores) AS participantes, COUNT(DISTINCT interesses.utilizadores_id_utilizadores) AS interessados,lat,lng,facebook_id,icons.classe AS tipo_classe FROM eventos LEFT OUTER JOIN tipo_evento ON tipo_evento.id_tipo_evento=eventos.tipo_evento_id_tipo_evento LEFT OUTER JOIN localizacao ON localizacao.localizacao = eventos.localizacao_localizacao LEFT OUTER JOIN participantes ON participantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN validaparticipantes ON validaparticipantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN interesses ON interesses.eventos_id_eventos = eventos.id_eventos  LEFT OUTER JOIN icons ON tipo_evento.icons_id = icons.id_icons WHERE (nome_evento LIKE :msg OR tipo_evento.nome_tipo_evento LIKE :msg OR localizacao.nome LIKE :msg)  AND eventos.ativo =1 AND data_evento > CAST(CURRENT_TIMESTAMP AS DATE) GROUP BY eventos.id_eventos  ORDER BY $passar   
)DUMMY_ALIAS1
UNION ALL 
SELECT id_eventos, criador,nome_evento,nome_tipo_evento,morada,data_evento,descricao_short, inscritos,  interessados,participantes,lat,lng,facebook_id, tipo_classe FROM(
SELECT id_eventos,eventos.utilizadores_id_utilizadores AS criador,nome_evento,tipo_evento.nome_tipo_evento,localizacao.nome AS morada,data_evento,descricao_short, COUNT(DISTINCT participantes.utilizadores_id_utilizadores) AS inscritos,COUNT(DISTINCT validaparticipantes.utilizadores_id_utilizadores) AS participantes, COUNT(DISTINCT interesses.utilizadores_id_utilizadores) AS interessados,lat,lng,facebook_id,icons.classe AS tipo_classe FROM eventos LEFT OUTER JOIN tipo_evento ON tipo_evento.id_tipo_evento=eventos.tipo_evento_id_tipo_evento LEFT OUTER JOIN localizacao ON localizacao.localizacao = eventos.localizacao_localizacao LEFT OUTER JOIN participantes ON participantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN validaparticipantes ON validaparticipantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN interesses ON interesses.eventos_id_eventos = eventos.id_eventos  LEFT OUTER JOIN icons ON tipo_evento.icons_id = icons.id_icons WHERE (nome_evento LIKE :msg OR tipo_evento.nome_tipo_evento LIKE :msg OR localizacao.nome LIKE :msg)  AND eventos.ativo =1 AND data_evento < CAST(CURRENT_TIMESTAMP AS DATE) GROUP BY eventos.id_eventos  ORDER BY $passar DESC 
...
UNION ALL 
SELECT id_eventos, criador,nome_evento,nome_tipo_evento,morada,data_evento,descricao_short, inscritos,  interessados,participantes,lat,lng,facebook_id, tipo_classe FROM(
SELECT id_eventos,eventos.utilizadores_id_utilizadores AS criador,nome_evento,tipo_evento.nome_tipo_evento,localizacao.nome AS morada,data_evento,descricao_short, COUNT(DISTINCT participantes.utilizadores_id_utilizadores) AS inscritos,COUNT(DISTINCT validaparticipantes.utilizadores_id_utilizadores) AS participantes, COUNT(DISTINCT interesses.utilizadores_id_utilizadores) AS interessados,lat,lng,facebook_id,icons.classe AS tipo_classe FROM eventos LEFT OUTER JOIN tipo_evento ON tipo_evento.id_tipo_evento=eventos.tipo_evento_id_tipo_evento LEFT OUTER JOIN localizacao ON localizacao.localizacao = eventos.localizacao_localizacao LEFT OUTER JOIN participantes ON participantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN validaparticipantes ON validaparticipantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN interesses ON interesses.eventos_id_eventos = eventos.id_eventos  LEFT OUTER JOIN icons ON tipo_evento.icons_id = icons.id_icons WHERE (nome_evento LIKE :msg OR tipo_evento.nome_tipo_evento LIKE :msg OR localizacao.nome LIKE :msg)  AND eventos.ativo =1 AND data_evento IS NULL GROUP BY eventos.id_eventos 
) DUMMY_ALIAS3 ) as resutl_table  LIMIT :limit, :results
...
UNION ALL 
SELECT id_eventos, criador,nome_evento,nome_tipo_evento,morada,data_evento,descricao_short, inscritos,  interessados,participantes,lat,lng,facebook_id, tipo_classe FROM(
SELECT id_eventos,eventos.utilizadores_id_utilizadores AS criador,nome_evento,tipo_evento.nome_tipo_evento,localizacao.nome AS morada,data_evento,descricao_short, COUNT(DISTINCT participantes.utilizadores_id_utilizadores) AS inscritos,COUNT(DISTINCT validaparticipantes.utilizadores_id_utilizadores) AS participantes, COUNT(DISTINCT interesses.utilizadores_id_utilizadores) AS interessados,lat,lng,facebook_id,icons.classe AS tipo_classe FROM eventos LEFT OUTER JOIN tipo_evento ON tipo_evento.id_tipo_evento=eventos.tipo_evento_id_tipo_evento LEFT OUTER JOIN localizacao ON localizacao.localizacao = eventos.localizacao_localizacao LEFT OUTER JOIN participantes ON participantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN validaparticipantes ON validaparticipantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN interesses ON interesses.eventos_id_eventos = eventos.id_eventos  LEFT OUTER JOIN icons ON tipo_evento.icons_id = icons.id_icons WHERE (nome_evento LIKE :msg OR tipo_evento.nome_tipo_evento LIKE :msg OR localizacao.nome LIKE :msg)  AND eventos.ativo =1 AND data_evento < CAST(CURRENT_TIMESTAMP AS DATE) GROUP BY eventos.id_eventos DESC ORDER BY $passar DESC 
) DUMMY_ALIAS2
UNION ALL 
SELECT id_eventos, criador,nome_evento,nome_tipo_evento,morada,data_evento,descricao_short, inscritos,  interessados,participantes,lat,lng,facebook_id, tipo_classe FROM(
SELECT id_eventos,eventos.utilizadores_id_utilizadores AS criador,nome_evento,tipo_evento.nome_tipo_evento,localizacao.nome AS morada,data_evento,descricao_short, COUNT(DISTINCT participantes.utilizadores_id_utilizadores) AS inscritos,COUNT(DISTINCT validaparticipantes.utilizadores_id_utilizadores) AS participantes, COUNT(DISTINCT interesses.utilizadores_id_utilizadores) AS interessados,lat,lng,facebook_id,icons.classe AS tipo_classe FROM eventos LEFT OUTER JOIN tipo_evento ON tipo_evento.id_tipo_evento=eventos.tipo_evento_id_tipo_evento LEFT OUTER JOIN localizacao ON localizacao.localizacao = eventos.localizacao_localizacao LEFT OUTER JOIN participantes ON participantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN validaparticipantes ON validaparticipantes.eventos_id_eventos = eventos.id_eventos LEFT OUTER JOIN interesses ON interesses.eventos_id_eventos = eventos.id_eventos  LEFT OUTER JOIN icons ON tipo_evento.icons_id = icons.id_icons WHERE (nome_evento LIKE :msg OR tipo_evento.nome_tipo_evento LIKE :msg OR localizacao.nome LIKE :msg)  AND eventos.ativo =1 AND data_evento IS NULL GROUP BY eventos.id_eventos 
