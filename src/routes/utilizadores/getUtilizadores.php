    ...
    if (Token::validarScopes('admin')) {
        ...
        if ($page > 0 && $results > 0) {
            ...
            if ($order == $orderDefault) {
                $sql = "SELECT id_utilizadores,utilizadores.nome,utilizadores.apelido,utilizadores.genero,utilizadores.data_nascimento,utilizadores.data_registo_user,utilizadores.email,utilizadores.foto,utilizadores.sobre,utilizadores.sobre_mini,utilizadores.telemovel,utilizadores.localizacao_id_localizacao,utilizadores.ativo,utilizadores.estatutos_id_estatutos,localizacao.nome AS morada,localizacao.lat,localizacao.lng,estatutos.nome_estatuto FROM `utilizadores` LEFT OUTER JOIN localizacao ON utilizadores.localizacao_id_localizacao = localizacao.localizacao LEFT OUTER JOIN estatutos ON utilizadores.estatutos_id_estatutos = estatutos.id_estatutos WHERE (utilizadores.`id_utilizadores` LIKE :msg OR utilizadores.`nome` LIKE :msg OR `apelido`LIKE :msg OR `data_nascimento`LIKE :msg OR `data_registo_user` LIKE :msg OR email LIKE :msg OR telemovel LIKE :msg AND localizacao.nome LIKE :msg OR estatutos.nome_estatuto LIKE :msg) AND id_utilizadores LIKE :id $extraWhere ORDER BY $passar  LIMIT :limit, :results";
            } else {
...
$app->get('/utilizadores/{id}', function (Request $request, Response $response) {
    ...
    if (Token::validarScopes('admin', $idUtilizador)) {
        if (is_int($idUtilizador) && $idUtilizador > 0) {
            ...
            $sql = "SELECT id_utilizadores,utilizadores.nome,utilizadores.apelido,utilizadores.genero,utilizadores.data_nascimento,utilizadores.data_registo_user,utilizadores.email,utilizadores.foto,utilizadores.sobre,utilizadores.sobre_mini,utilizadores.telemovel,utilizadores.localizacao_id_localizacao,utilizadores.ativo,utilizadores.estatutos_id_estatutos,localizacao.nome AS  morada,localizacao.lat,localizacao.lng,estatutos.nome_estatuto FROM `utilizadores` LEFT OUTER JOIN localizacao ON localizacao.localizacao = utilizadores.localizacao_id_localizacao LEFT OUTER JOIN estatutos ON estatutos.id_estatutos = utilizadores.estatutos_id_estatutos  WHERE id_utilizadores = :id ";
